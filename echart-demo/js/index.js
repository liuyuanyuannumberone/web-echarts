        ...
        xAxis: [
            {
                ...
                axisTick: {
                    alignWithLabel: false, //刻度在柱子中间显示，不好看
...
(function () {
    ...
    $(".sales .caption").on("click", "a", function () {
        index = $(this).index() - 1; // 此时要注意这个索引号的问题
        $(this)
            .addClass("active")
            .siblings("a")
            .removeClass("active");
        var arr = data[this.dataset.type]; //// 根据拿到的值 去找数据
        option.series[0].data = arr[0];
        option.series[1].data = arr[1];
        myChart.setOption(option);
    });  //注册了一个点击事件

    //  tab栏自动切换效果  开启定时器每隔3s，自动让a触发点击事件即可
        ...
        series: [
            {
                ...
                type: "pie",
                radius: ["130%", "150%"],
                center: ["48%", "80%"],   // 移动下位置  套住50%文字
                labelLine: {
                    normal: {
                        show: false
                    }
                },
                startAngle: 180, // 饼形图的起始角度为 180， 注意不是旋转角度
                hoverOffset: 0,   // 鼠标经过不需要放大偏移图形
                data: [
                    {
                        value: 100,
                        itemStyle: {
